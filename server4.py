import json, os, threading, sqlite3, time, io, traceback, subprocess
...
# -------------------- Jira helpers (CSV + jiraprint) --------------------

def _jiraprint_fetch(jira_id: str):
    """
    Optional: jiraprint --id JIRA-1234 -s SHORTERM
    ...
    try:
        proc = subprocess.run(
            ["jiraprint", "--id", jira_id, "-s", "SHORTERM"],
        ...
        return {"key": jira_id, "summary": summary, "description": desc}
    except Exception:
        return None

def _jira_from_index(jira_id: str):
    if not jira_id:
        return None
    JIRA_INDEX._ensure_loaded()
    for m in JIRA_INDEX.meta:
        mid = (m.get("id") or m.get("key") or "").strip().upper()
        if mid == jira_id.strip().upper():
            return {
                "key": jira_id,
                "summary": m.get("summary",""),
                "description": m.get("description","") or m.get("text","")
            }
    return None
...
def fetch_jira_issue(jira_id: str):
    """
    Order:
      1) jiraprint CLI (if configured)
      2) Jira CSV BGE index
      3) Fallback 'Unknown Jira issue'
    Demo mapping intentionally removed for production-like flow.
    """
    if not jira_id:
        return None
    issue = _jiraprint_fetch(jira_id)
    if issue:
        return issue
    issue = _jira_from_index(jira_id)
    if issue:
        return issue
    return {
        "key": jira_id,
        "summary": "Unknown Jira issue",
        "description": "No Jira backend configured or CSV not trained yet."
    }
...
def agent_planner(text):
    ...
    Decide worker:
      - jira         (Jira flow)
      - ccf_generate (CCF generation)
      - ccf_opt      (CCF optimisation)
      - pdf          (PDF QA)
      - sv_rewrite   (SV/UVM refactor)
...
class XgentHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # ---------- Jira CSV training ----------
            if parsed.path=="/api/jira_upload_train":
                ...
                for r in reader:
                    jira_id = r.get("key") or r.get("jira_id") or r.get("id") or ""
                    if not jira_id:
                        continue
                    ...
                    text = " ".join([jira_id, summary, desc, extra])
                    meta = {
                        "id": jira_id,
                        "summary": summary,
                        "description": desc,
                ...
                if not rows:
                    return self._send_json(200,{"ok":False,"error":"empty_or_invalid_csv"})
                texts = [t for (t,_) in rows]
                metas = [m for (_,m) in rows]
                JIRA_INDEX.add(texts, metas)
            ...
            if parsed.path=="/api/jira_match":
                ...
                hits=JIRA_INDEX.search(q,top_k=int(payload.get("top_k",5)))
                return self._send_json(200,{"ok":True,"results":hits})

            if parsed.path=="/api/jira_flow":
                jira_id=(payload.get("jira_id") or "").strip()
                override=(payload.get("description_override") or "").strip()
                if not jira_id and not override:
                    return self._send_json(200,{"ok":False,"error":"jira_id_or_description_required"})
                issue = fetch_jira_issue(jira_id) if jira_id else {"key":"USER","summary":"","description":override}
                desc = (issue.get("summary","")+"\n"+issue.get("description","")).strip()
                if override:
                    desc = override + "\n\nOriginal Jira:\n" + desc

                # RAG over Jira + testcases
                try:
                    similar_jira = JIRA_INDEX.search(desc, top_k=5)
                except Exception:
                    similar_jira = []
                ...
                best = similar_jira[0]["score"] if similar_jira else 0.0
                if best < thr and not testcases:
                    flow_action = "codegen"
                elif best < thr:
                    flow_action = "testcase_match"
                else:
                    flow_action = "jira_match"

                # Solutions RAG (learning index)
                try:
                ...
                prompt = (
                    ...
                    "Given the Jira description and retrieved context, propose:\n"
                    "- likely root cause\n"
                    "- key debug steps\n"
                    "- recommended UVM testcase (reuse or new)\n"
                    "- performance/coverage/memory hints if relevant.\n\n"
                    f"Jira {issue.get('key')}:\n"
                    f"{issue.get('summary')}\n{issue.get('description')}\n\n"
                    f"Flow action: {flow_action}, threshold={thr}\n\n"
                    f"Similar Jira:\n{json.dumps(similar_jira,indent=2)}\n\n"
                    f"Candidate testcases:\n{json.dumps(testcases,indent=2)}\n\n"
            ...
            # ---------- Debug script (debug_recipes YAML) ----------
            if parsed.path=="/api/debug_script":
                jira_id = (payload.get("jira_id") or "").strip()
                desc = (payload.get("description") or "").strip()
                env_info = payload.get("env", {})
                issue = fetch_jira_issue(jira_id) if jira_id else {"key":"USER","summary":"","description":desc}
                ...
                categories = []
                if "slow" in low or "perf" in low or "performance" in low:
                    categories.append("perf")
                if "oom" in low or "out of memory" in low or "memory" in low:
                    categories.append("memory")
                if "coverage" in low or "ccf" in low:
            ...
            # ---------- CCF optimisation (text-based) ----------
            if parsed.path=="/api/ccf_opt":
                ccf_txt = (payload.get("text") or "").strip()
                if not ccf_txt:
                    return self._send_json(200,{"ok":False,"error":"empty_ccf"})
                ctx = gather_contexts(ccf_txt, top_k_each=2)
                prompt = (
                    "You are a coverage engineer. Analyse the following CCF and:\n"
                    "- identify redundant/overlapping bins/crosses\n"
                    "- suggest merges / deletions\n"
                    "- propose a cleaned-up CCF version.\n\n"
                )
                if ctx:
                    prompt += "Context (Jira/Testcases/PDF/Solutions):\n"+ctx+"\n\n"
                prompt += "Original CCF:\n"+ccf_txt+"\n\nOptimised CCF and explanation:\n"
                out = llm_generate(prompt)
