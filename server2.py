...
# -------------------- Jira helpers (CSV + jiraprint + demo) --------------------
...
def _demo_dummy_jira_issue(jira_id: str):
    mapping = {
        ...
        "AVSREQ-000002": {
            ...
            "description": "Coverage report shows 0% hits on cross cp_lenXprot in 'axi_cov_ccf'."
        },
        "AVSREQ-000003": {
            "summary": "Out-of-memory when dumping large waves with assertion coverage enabled",
            "description": "Xcelium run OOMs on 128G box when FSDB + assertion coverage both enabled."
        },
        "AVSREQ-000004": {
        ...
        "AVSREQ-000006": {
            ...
            "description": "Core/fabric/top CCFs contain large overlapping coverpoints and crosses."
        },
        "AVSREQ-000007": {
            "summary": "CDC assertions firing only under -newperf",
            "description": "CDC assertions fail only when -newperf is used; -disable_opt changes behaviour."
        },
    }
...
def _jiraprint_fetch(jira_id: str):
    """
    Try external CLI: jiraprint --id JIRA-1234 -s SHORTERM
    Expected to return some human-readable text. We parse first line as summary, rest as description.
    """
    if not jira_id:
        return None
    try:
...
def _jira_from_index(jira_id: str):
    """
    Look into JIRA_INDEX.meta for a matching id/key and use that as Jira issue.
    """
    if not jira_id:
        return None
...
def fetch_jira_issue(jira_id: str):
    """
    Order of preference:
      1) jiraprint CLI (real Jira)
      2) Jira CSV index (trained JIRA_INDEX)
      3) Demo hardcoded mapping
      4) Fallback 'Unknown issue'
    """
    if not jira_id:
        return None
    ...
    if issue:
        return issue
    issue = _demo_dummy_jira_issue(jira_id)
    if issue:
        return issue
    return {
        "key": jira_id,
...
def agent_planner(text):
    ...
    Decide what to do with the user input:
      - jira         (Jira flow via /api/jira_flow)
